...
// 将 getFestivalRecipes 函数移到组件外部并导出
...
const MyFavorites: React.FC = () => {
    ...
    const favoriteRecipes = [
        ...
        ...todayRecipes.map((recipe, index) => ({
            ...
            type: 'today' as const,
            index
        })).filter((_, index) => todayFavoriteIds.includes(index)),

        // 节日食谱收藏项
        ...festivalRecipes.map((recipe, index) => ({
            ...recipe,
            type: 'festival' as const,
            index
        })).filter((_, index) => festivalFavoriteIds.includes(index))
    ];

    // 取消收藏处理函数
    const handleUnfavorite = (type: 'today' | 'festival', index: number) => {
        if (type === 'today') {
            removeFromTodayFavorites(index);
        } else {
            removeFromFestivalFavorites(index);
        }
    };
